...
const Content = () => {
  ...
  const generateNavigationUrl = (id) => `/board/${role}/${id}`;
  ...
  useEffect(() => {
    const fetchBoardData = async () => {
      ...
      try {
        const boardCache = localStorage.getItem(`board_${id}`);
        if (boardCache) {
          const cachedData = JSON.parse(boardCache);
          setBoard(cachedData.board);
          setPreviousId(cachedData.previousId);
          setNextId(cachedData.nextId);
          setImages(cachedData.images || []);
          setFiles(cachedData.files || []);
        } else {
          const [boardRes, navRes, imagesRes, filesRes] = await Promise.all([
            fetchData(`https://dcs-site-5dccc5b2f0e4.herokuapp.com/api/board/role/${role}/${id}`),
            fetchData(`https://dcs-site-5dccc5b2f0e4.herokuapp.com/api/board/${role}/${id}/btn`),
            fetchData(`https://dcs-site-5dccc5b2f0e4.herokuapp.com/api/images/board/${id}`),
            fetchData(`https://dcs-site-5dccc5b2f0e4.herokuapp.com/api/files/board/${id}`),
          ]);
  
          const boardData = {
            board: boardRes,
            previousId: navRes?.prev?.id || null,
            nextId: navRes?.next?.id || null,
            images: imagesRes,
            files: filesRes,
          };
    ...
    <div className="intro">
      ...
      {board ? (
       ...
       <div className="btn_box1">
        ...
        <Button onClick={() => navigate(generateNavigationUrl(previousId))} disabled={!previousId}>
          이전글
        </Button>
    
     
        <Button onClick={() => navigate(generateNavigationUrl(nextId))} disabled={!nextId}>
          다음글
        </Button>
